...
function Home() {
  ...
  async function deleteHistory(id) {
    try {
      const res = await fetch(
        `https://geofinder-api.vercel.app/api/history?id=${id}`,
        {
          method: "DELETE",
        },
      );
  ...
  return (
    <>
      <div id="upper">
        <h1 id="geoFinder2" onClick={handleLogout}>
          GeoFinder
        </h1>
        <input
          type="text"
        ...
        />
        <button className="upperBtn" onClick={searchIP}>
          Search
        </button>
        <button className="upperBtn" onClick={clearIP}>
          Clear
        </button>
      </div>
